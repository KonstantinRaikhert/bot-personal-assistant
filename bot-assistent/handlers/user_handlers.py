...
async def process_beginning_command(message: Message):
    ...
    await message.answer(
        ...
        reply_markup=create_pagination_keyboard(
            "backward", f'{users_db[message.from_user.id]["page"]}/{len(book)}', "forward"
        ),
    )
...
async def process_forward_press(callback: CallbackQuery):
    if users_db[callback.from_user.id]["page"] < len(book):
        ...
        await callback.message.edit_text(
            text=text,
            reply_markup=create_pagination_keyboard(
                "backward", f'{users_db[callback.from_user.id]["page"]}/{len(book)}', "forward"
            ),
        )
...
async def process_edit_press(callback: CallbackQuery):
    await callback.message.edit_text(
        text=LEXICON[callback.data], reply_markup=create_edit_keyboard(*users_db[callback.from_user.id]["bookmarks"])
    )
    await callback.answer()
...
async def process_del_bookmark_press(callback: CallbackQuery):
    ...
    if users_db[callback.from_user.id]["bookmarks"]:
        await callback.message.edit_text(
            text=LEXICON["/bookmarks"], reply_markup=create_edit_keyboard(*users_db[callback.from_user.id]["bookmarks"])
        )
    else:
